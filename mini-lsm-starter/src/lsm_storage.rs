...
impl LsmStorageInner {
    ...
    pub fn force_freeze_memtable(&self, _state_lock_observer: &MutexGuard<'_, ()>) -> Result<()> {
        ...
        let inner = Arc::make_mut(&mut inner_state);

        let new_table = MemTable::create(inner.memtable.id() + 1);
        inner.imm_memtables.insert(0, inner.memtable.clone());
        inner.memtable = Arc::new(new_table);
        *state = Arc::new(inner.clone());
        Ok(())
    }
